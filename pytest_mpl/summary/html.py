...
class Result:
    ...
    def badges(self):
        ...
        for test_type, status_getter in [
            ("image", image_status_msg),
            ("hash", hash_status_msg),
        ]:
            status = getattr(self, f"{test_type}_status")
            if (
                (status == "missing")
                or (self.status == "failed" and status == "match")
                or (self.status == "passed" and status == "diff")
            ):  # Only show if different to overall status
                yield {
                    "status": status,
                    "svg": test_type,
                    "tooltip": status_getter(status),
                }
